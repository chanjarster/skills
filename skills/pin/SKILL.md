---
name: pin
description: 管理项目情报（Project Information）文档系统。当需要创建、读取、更新或导航项目文档时使用，包括业务知识（需求文档、操作手册）、技术文档（架构、开发环境、代码地图）、规范文档（git 规范、代码规范、示例）和任务日志。在项目初始化、编写需求/技术/规范文档、管理开发任务、或需要了解项目上下文时触发。
---
...
Pin 是一个结构化的项目文档管理系统，以 `pin-docs/` 为根目录，帮助 AI 从业务、技术、规范、任务四个维度理解和管理项目信息。
...
│   ├── 1-需求文档/    # PRD，按业务领域组织
│   └── 2-操作手册/    # 用户视角的操作指南
├── 技术文档-X/        # 技术架构、开发环境、代码地图
│   ├── 0-索引.md
│   ├── 1-技术架构.md
│   ├── 2-开发环境.md
│   └── 3-代码地图/
├── 规范文档-X/        # git 规范、代码规范、示例
│   ├── 0-索引.md
│   ├── 1-git 规范.md
│   ├── 2-代码规范.md
│   └── 3-示例/
...
2. 如果不存在，按以下步骤创建目录骨架：
   - 创建四大类目录：`业务知识/`、`技术文档-X/`、`规范文档-X/`、`任务日志/`
   - 创建各子类目的子目录（参见上方目录树）
   - 创建 `assets/` 目录
3. 为每个大类创建对应的 `0-索引.md`（参考 `references/index-template.md`）
4. 创建 `业务知识/1-需求文档/0-overview.md`（参考 `references/templates/overview.md`）
...
例如搜索"用户登录"相关文档：
- 先读 `业务知识/0-索引.md` 找到相关需求文档
- 再读 `技术文档-X/0-索引.md` 找到相关代码地图
- 最后读取具体文档内容
...
**创建新任务**：
1. 在 `任务日志/0-未开始/` 下创建目录 `<yyMMdd>-<HHmm>-<任务标题>`
...
- `1-git 规范.md`：分支规范、commit 规范。模板：`references/templates/git-convention.md`
...
**模板**：`references/index-template.md`
...
**模板**：`references/templates/` 目录

### assets

**用途**：存放图片、截图等公共资源。

文档中需要引用图片时，使用相对路径（如 `../assets/login-flow.png`）指向该目录下的文件。
...
5. **公共资源**：图片、截图等存放在 `assets/` 目录下，文档中使用相对路径引用
