...
2. 如果不存在，按以下步骤创建目录骨架：
   ...
   - 创建各子类目的子目录（参见上方目录树），所有在目录树中已定义的固定子目录均需预先创建；含 `<占位符>` 的目录为动态目录，在实际使用时按需创建，初始化阶段跳过。
3. 为每个大类创建对应的 `0-索引.md`（格式严格遵守 `references/templates/index-template.md`）
...
1. 先读取现有内容
2. 在对应章节增量更新，保持文档精简
3. 更新所在层级的 `0-索引.md`，更新被修改的文档的索引项。若发现索引与实际文档内容不一致，以文档内容为准并同步修正索引
